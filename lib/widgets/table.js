...
class Table extends Box {
  ...
  constructor(options = {}) {
    ...
    if (options.scrollable && !options._tableInit) {
      options._ignore = false; // Let Element create ScrollableBox
      const instance = super(options);

      // Copy Table methods to the ScrollableBox instance
      if (instance instanceof ScrollableBox) {
        instance._calculateMaxes = Table.prototype._calculateMaxes;
        instance.setData = Table.prototype.setData;
        Object.defineProperty(instance, 'setRows', {
          get() { return this.setData; }
        });
        instance.render = Table.prototype.render;
        instance.type = 'table';

        // Set pad property
        instance.pad = options.pad != null
          ? options.pad
          : 2;

        // Initialize data
        instance.setData(options.rows || options.data);

        // Set up event listeners
        instance.on('attach', () => {
          instance.setContent('');
          instance.setData(instance.rows);
        });
