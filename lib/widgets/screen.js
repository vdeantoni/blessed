...
class Screen extends Node {
  ...
  focusOffset(offset) {
    const shown = this.keyable.filter((el) => {
  ...
  readEditor(options, callback) {
    ...
    return writeFile((err) => {
      ...
      return this.exec(editor, args, opt, (err, success) => {
        if (err) return callback(err);
...
Screen.prototype.resetCursor = Screen.prototype.cursorReset;
...
const registry = Node.ScreenRegistry;
...
Object.defineProperty(Screen, 'global', {
  ...
  set(value) { registry.global = value; }
});
...
Screen.bind = function(screen) {
  ...
  process.on('uncaughtException', Screen._exceptionHandler = function(err) {
    ...
    registry.instances.slice().forEach(function(screen) {
      screen.destroy();
    });
