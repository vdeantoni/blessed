...
class ScrollableBox extends Box {
  ...
  /**
   * Static method to initialize scrollable behavior on any element
   * @param {Element} element - The element to make scrollable
   * @param {Object} options - Scrollable options
   */
  static initScrollable(element, options = {}) {
    ...
    if (element.scrollbar) {
      ...
      element.style.scrollbar = element.style.scrollbar || element.scrollbar.style;
      ...
      if (element.track || element.scrollbar.track) {
        ...
        element.style.track = element.style.scrollbar.track || element.style.track;
      ...
      if (options.mouse) {
        element.on('mousedown', (data) => {
          ...
          if (x === element.width - element.iright - 1) {
            ...
            element.onScreenEvent('mousedown', smd = (data) => {
              const y = data.y - element.atop;
            ...
            // occurs.
            element.onScreenEvent('mouseup', smu = () => {
    ...
    // Add scroll methods to the element if not already present
    const methods = [
      '_scrollBottom', 'scrollTo', 'getScroll', 'scroll',
      '_recalculateIndex', 'resetScroll', 'getScrollHeight',
      'getScrollPerc', 'setScrollPerc'
    ];

    methods.forEach(method => {
      if (!element[method]) {
        element[method] = ScrollableBox.prototype[method];
      }
    });
  ...
  constructor(options = {}) {
    super(options);
    ScrollableBox.initScrollable(this, options);
  }
