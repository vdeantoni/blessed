...
class Element extends Node {
  ...
  constructor(options = {}) {
    ...
    if (options.scrollable && !options._ignore && (!options.type || (options.type !== 'scrollable-box'))) {
      const ScrollableBox = require('./scrollablebox');
      options._ignore = true;
      const instance = new ScrollableBox(options);
      return instance;
    }
    ...
    [['hoverEffects', 'mouseover', 'mouseout', '_htemp'],
     ['focusEffects', 'focus', 'blur', '_ftemp']].forEach((props) => {
...
main:
  ...
  enableDrag(verify) {
    ...
    this.on('mousedown', this._dragMD = (data) => {
    ...
    this.onScreenEvent('mouse', this._dragM = (data) => {
