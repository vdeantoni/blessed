# Phase 3C: Type Refinement Strategy

## Learnings from Initial Attempt

### What We Discovered

**Problem with Simple Type Replacement:**
- Attempted to replace `any` with `Node` type in base class
- **Result:** 558 cascading errors across the codebase
- **Root Cause:** Widget hierarchy complexity

**Key Issues:**
1. **Screen Property Typing:**
   - Declared as `screen: any` in Node
   - Actually holds a `Screen` instance (subclass of Node)
   - `Screen` has methods like `clearRegion()`, `render()`, `program`
   - Typing as `Node` loses these methods → errors everywhere

2. **Parent Property Typing:**
   - Could be any Element/Box/List/etc (all Node subclasses)
   - Need to preserve specific subclass methods

3. **Circular Dependencies:**
   - Node → Screen (for screen property)
   - Screen → Node (extends Node)
   - Can't simply import types without careful design

### Statistics

**Current `any` Usage:**
- Total: ~675 occurrences
- screen.ts: 114
- element.ts: 91
- node.ts: 39
- tput.ts: 59 (low-level, defer)
- program.ts: 50 (low-level, defer)

**Common Patterns:**
- `el: any` (33×) - widget references
- `data: any` (9×) - user data
- `pos: any` (3×) - position objects

## Recommended Strategy

### Approach 1: Incremental Non-Hierarchical Types (RECOMMENDED)

Start with types that don't involve the widget hierarchy:

**Phase 1: Data Structures** (Week 1)
- ✅ Position, Coords, Padding, Border (already done in common.ts)
- Replace `pos: any` → `Position | Coords`
- Replace `options: any` → specific option types
- Replace `data: any` → `Record<string, unknown>`
- **Benefit:** 50-100 `any` replacements with minimal risk

**Phase 2: Event Handlers** (Week 1-2)
- Type event handler parameters
- Replace `key: any` → proper Key interface
- Replace event data: `any` → specific event types
- **Benefit:** 30-50 replacements, better autocomplete

**Phase 3: Utility Functions** (Week 2)
- Type helpers in helpers.ts
- Type color functions
- Type unicode utilities
- **Benefit:** 20-30 replacements

**Phase 4: Strategic `any` for Hierarchy** (Week 3)
- Accept that widget tree references stay `any` for now
- Document why (circular deps, complexity)
- Add JSDoc comments explaining actual types
- **Benefit:** Honest about limitations, no cascade errors

### Approach 2: Generics with Constraints (ADVANCED)

Use TypeScript generics to properly type hierarchy:

```typescript
class Node<
  TScreen extends Node = any,
  TParent extends Node = any
> extends EventEmitter {
  screen: TScreen;
  parent: TParent;
  children: Node<TScreen, this>[];

  // Methods use the generic types
  append<T extends Node<TScreen, this>>(element: T): void {
    // ...
  }
}

class Screen extends Node<Screen, null> {
  // Screen-specific methods
  clearRegion(): void { }
  render(): void { }
}

class Element extends Node<Screen, Element> {
  // Element-specific
}
```
...
### Approach 3: Discriminated Unions (ALTERNATIVE)
...
function isScreen(node: BlessedWidget): node is Screen {
  return node.type === 'screen';
}

// Usage
if (isScreen(element.screen)) {
  element.screen.clearRegion(); // TypeScript knows it's Screen
}
```
...
- Union becomes large (44+ widgets)

## Community Types Analysis

### Key Discoveries from @types/blessed

Analyzed the community-created TypeScript definitions for blessed to find proven patterns that work well in practice.

**✅ Validation:** The community types **confirm our Phase 3C.1 strategy is correct!**

### Concrete Patterns Found

#### 1. Event Type Definitions
```typescript
// From community types - directly applicable!
interface IKeyEventArg {
  full: string;        // Full key sequence
...
interface IMouseEventArg {
  x: number;
  y: number;
  action: "mousedown" | "mouseup" | "mousemove";  // Union type!
}
...
on(event: "keypress", callback: (ch: string, key: IKeyEventArg) => void): this;
on(event: NodeMouseEventType, callback: (arg: IMouseEventArg) => void): this;
```

**Impact:** Replaces ~30-50 `key: any` and event `any` parameters

#### 2. Position/Coordinate Types
```typescript
// Already in our common.ts!
interface Position {
  left: number | string;    // Supports "50%", "center", pixels
  right: number | string;
  top: number | string;
...
class PositionCoords {
  xi: number;  // Internal x coordinate
  xl: number;  // x limit
  yi: number;  // Internal y coordinate
  yl: number;  // y limit
}
...
**Impact:** Replace `pos: any` → `Position | Coords` (10-15 occurrences)

#### 3. Data Property Pattern
```typescript
// Better than plain `any`
...
**Impact:** Replace 9 `data: any` occurrences

#### 4. Style Object Pattern
```typescript
interface TStyle {
  bg?: string;
  fg?: string;
  ...
  underline?: boolean;
  border?: TStyleBorder;
  hover?: Effects;      // Nested typed objects
  focus?: Effects;
  scrollbar?: { bg?: string; fg?: string };
}

// Widget-specific via intersection
type ListElementStyle = {
  selected?: any;  // Still uses `any` for truly dynamic properties!
  item?: any;
} & TStyle;
```

**Key Insight:** Community types **still use `any`** for truly dynamic properties. Don't aim for zero `any`!

#### 5. Options Pattern
```typescript
// Every widget has strongly-typed options
interface IHasOptions<T extends IOptions> {
  options: T;
}

interface BoxOptions extends ElementOptions {
  border?: Border | "line" | "bg";  // Union for flexibility
  padding?: number | Padding;        // Number for uniform, object for detailed
  scrollable?: boolean;
  // ... all optional fields
}
```

**Impact:** Better autocomplete, catches invalid options at compile time

#### 6. Flexible Input Types
```typescript
// Accept multiple formats for developer convenience
border?: Border | "line" | "bg";           // Object or string shorthand
padding?: number | Padding;                 // Uniform or detailed
align?: "left" | "center" | "right";       // String unions
```

### Important Philosophy from Community Types

**Pragmatic `any` Usage:** The community types show it's acceptable to keep `any` for:
- Truly dynamic widget properties (`selected?: any`, `item?: any`)
- Complex callback contexts
- Low-level terminal code
- Widget hierarchy in some places (they use generics with `any` defaults!)

**Lesson:** Aim for **strategic typing** where it adds value, not zero `any`.

---

## Recommended Implementation Plan

### Phase 3C.1: Low-Hanging Fruit (2-3 weeks)

**Goal:** Replace 100-150 `any` types with minimal risk using proven community patterns

#### Week 1: Data Structures & Events (50-70 replacements)

**Create Event Interfaces:**
```typescript
// packages/blessed/lib/types/events.ts
...
export interface MouseEvent {
  x: number;
  y: number;
  action: "mousedown" | "mouseup" | "mousemove" | "mousewheel" | "wheeldown" | "wheelup";
  shift?: boolean;
  ctrl?: boolean;
  meta?: boolean;
}
...
**Tasks:**
- [ ] Create `events.ts` with KeyEvent and MouseEvent interfaces
- [ ] Replace `data: any` → `Record<string, unknown>` (9 occurrences)
- [ ] Replace `pos: any` → `Position | Coords` (10-15 occurrences)
- [ ] Replace obvious event parameter `any` → `KeyEvent | MouseEvent`
- [ ] Tests: All 1,638 must pass
...
// After
on(event: "keypress", callback: (ch: string, key: KeyEvent) => void): this;
on(event: "mouse", callback: (event: MouseEvent) => void): this;
```

**Tasks:**
- [ ] Update all `on()` method signatures with specific event types
- [ ] Type key event handler callbacks: `(ch: string, key: KeyEvent) => void`
- [ ] Type mouse event handler callbacks: `(event: MouseEvent) => void`
- [ ] Create union types for event names where applicable
- [ ] Tests: All 1,638 must pass

#### Week 3: Style & Options (20-30 replacements)

**Create Base Style Interface:**
```typescript
// packages/blessed/lib/types/style.ts
...
**Tasks:**
- [ ] Create `style.ts` with base Style interface
- [ ] Enhance widget-specific Options interfaces
- [ ] Replace style `any` → `Style` or widget-specific style types
- [ ] Add JSDoc to remaining strategic `any` types (hierarchy, dynamic properties)
- [ ] Tests: All 1,638 must pass

**Completion Criteria:**
- [ ] 100-150 `any` types replaced
- [ ] No new TypeScript errors
- [ ] All 1,638 tests passing
- [ ] Widget hierarchy `any` types documented with JSDoc explaining why

### Phase 3C.2: Hierarchy Types (Future - if needed)

**Only pursue if:**
- User demand for better typing
- Time/resources available
- Team comfortable with generics

**Options:**
- Generics approach (4-6 weeks)
- Discriminated unions (2-3 weeks)
- Hybrid approach (3-4 weeks)

## Success Metrics

**Phase 3C.1 Goals:**
- ✅ 100-150 `any` → proper types (~20% reduction)
- ✅ All non-hierarchical `any` replaced
- ✅ Better developer experience (autocomplete, errors)
- ✅ Zero new bugs/regressions
- ✅ All 1,638 tests passing

**What We're NOT Fixing (Yet):**
- ❌ Widget hierarchy `any` (screen, parent, children)
- ❌ Low-level terminal code (tput, program)
- ❌ Complex internal state (`any` used intentionally)

## Decision Matrix

| Approach | Risk | Time | Value | Recommended |
|----------|------|------|-------|-------------|
| Non-hierarchical first | Low | 2-3 wk | High | ✅ Yes |
| Generics | Medium | 4-6 wk | High | ⏳ Later |
| Unions | Low | 2-3 wk | Medium | 🤔 Maybe |
| Keep hierarchy `any` | None | 0 wk | - | ✅ Yes (for now) |

## Next Steps

1. **Immediate:** Focus on Phase 3C.1 (non-hierarchical types)
2. **Document:** Add JSDoc to widget hierarchy `any` types
3. **Track:** Monitor which `any` types cause real issues
4. **Reassess:** After 3C.1, decide if hierarchy typing is worth it

---

## Reference: Community Types Snippets

For future reference, here are key patterns from the @types/blessed community types file:

### Complete Event Type Pattern
```typescript
namespace Events {
  interface IMouseEventArg {
    x: number;
...
### Flexible Padding/Border Pattern
```typescript
interface Padding {
  left?: number;
  right?: number;
...
// Accept either number (uniform) or object (detailed)
padding?: number | Padding;
border?: Border | "line" | "bg";
```

### Generic Node Pattern (for future Phase 3C.2)
```typescript
abstract class Node extends EventEmitter implements IHasOptions<INodeOptions>, IDestroyable {
  constructor(options: INodeOptions);

  screen: Screen;    // Simple typed property
  parent: Node;      // Base class type
  children: Node[];  // Array of base class

  // Methods work with Node type
...
**Key Takeaway:** Community types balance pragmatism (simple types, some `any`) with type safety (interfaces, unions, overloads).

### JSDoc Documentation from Community Types

The community types include comprehensive JSDoc comments that should be reused during implementation. These provide context about what properties do, including defaults, side effects, and usage notes.

**Key JSDoc patterns to apply:**
- Property descriptions with defaults (e.g., "Amount of time (in ms) to redraw the screen after the terminal is resized (Default: 300)")
- Method descriptions explaining behavior and side effects
- Parameter descriptions for complex options
- Event descriptions with trigger conditions
- Notes about experimental features or limitations
...
- Week 1: Add JSDoc to data structures and event interfaces
- Week 2: Add JSDoc to event handler signatures
- Week 3: Add JSDoc to remaining strategic `any` types explaining why they're kept as `any`

See the APPENDIX below for the complete set of JSDoc comments from the community types.

---

**Bottom Line:**
We can get 80% of the value (better typing, fewer `any`) with 20% of the effort (non-hierarchical types), while avoiding the risky widget hierarchy refactor.


### APPENDIX (community types)
