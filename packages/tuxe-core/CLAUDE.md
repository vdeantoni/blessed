# Claude Context for @tuxe/core

This document provides architectural context and development guidelines for working on the `@tuxe/core` package.

## Overview

`@tuxe/core` is the **platform-agnostic core** of the Tuxe terminal UI library. It contains all widget logic, rendering code, and terminal control primitives while having **zero dependencies** on any specific platform (Node.js, browser, etc.).
...
**Initialization:**
- Platform packages (@tuxe/node, @tuxe/browser) call `setRuntime()` at startup
...
**Solution**: Core package has zero dependencies, adapter packages (`@tuxe/node`, `@tuxe/browser`) provide implementations.
...
3. **Update imports**:
   ...
   // After
   import { Screen, Box, List } from '@tuxe/core';
   ```
...
- Look at platform adapter implementations (`@tuxe/node`, `@tuxe/browser`)
