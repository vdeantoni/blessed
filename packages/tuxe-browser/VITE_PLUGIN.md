# @tuxe/browser Vite Plugin

## The Problem

The browser runtime must be initialized **before** any `@tuxe/core` code executes. However, ESM import hoisting means imports are always executed before module-level code, making it impossible to guarantee initialization order with normal bundling.
...
import tuxeBrowser from '@tuxe/browser/vite-plugin';
...
3. **Main app loads** - Your bundled app loads and can safely use `@tuxe/core` widgets

## Plugin Options

```typescript
interface TuxeBrowserPluginOptions {
  /**
...
<head>
  <script type="module">
    ...
    import { BrowserRuntime } from '@tuxe/browser';
...
This means the runtime wasn't set up before `@tuxe/core` tried to use it. Make sure:
...
```
[tuxe-browser] Runtime initialized via vite plugin
```
