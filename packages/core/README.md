...
`@tuxe/core` uses a runtime abstraction layer that defines interfaces for all platform-specific operations:
...
Platform adapters (`@tuxe/node`, `@tuxe/browser`) provide concrete implementations:

```typescript
// @tuxe/node provides Node.js implementations
import { createNodeRuntime } from '@tuxe/node';
createNodeRuntime();

// @tuxe/browser provides browser polyfills
...
```typescript
import { ANSIImage } from '@tuxe/core';
...
```typescript
import { Form, Textbox, Button } from '@tuxe/core';
...
```typescript
import { ScrollableBox } from '@tuxe/core';
...
```typescript
import { Terminal } from '@tuxe/core';
...
When using `@tuxe/browser`, the library provides browser-compatible implementations using:
...
- [`@tuxe/browser`](../tuxe-browser) - Browser runtime adapter with xterm.js integration
...
Contributions are welcome! Please see the [main repository](https://github.com/vdeantoni/tuxe) for contribution guidelines.
