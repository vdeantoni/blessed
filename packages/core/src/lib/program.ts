...
class Program extends EventEmitterBase {
  ...
  [key: string]: any;
    ...
    constructor(options?: any, legacyOutput?: any) {
        ...
        this.isiTerm2 = termProgram === 'iTerm.app'
            || !!getEnvVar("ITERM_SESSION_ID");
        ...
        this.isLXDE = false;
        this.isVTE = !!getEnvVar("VTE_VERSION")
            || this.isXFCE
            || this.isTerminator
            || this.isLXDE;
    ...
    _listenInput() {
        ...
        this.input.on('data', this.input._dataHandler = (data: BufferType) => {
            Program.instances.forEach((program) => {
                if (program.input !== this.input) return;
