...
class Tput {
  ...
  _useXtermCap() {
    return this.injectTermcap(
      this.runtime.path.join(getDataPath(), "xterm.termcap"),
    );
  }
  ...
  _useInternalCap(name: string) {
    name = this.runtime.path.basename(name);
    return this.injectTermcap(
      this.runtime.path.join(getDataPath(), name + ".termcap"),
    );
  }
  ...
  readTerminfo(term: string) {
    ...
    file = this.runtime.path.normalize(this._prefix(term));
    data = this.runtime.fs.readFileSync(file);
  ...
  parseTerminfo(data: BufferType, file?: string) {
    var info: any = {},
      extended: any,
      l = data.length,
      i = 0,
    ...
    info.dir = this.runtime.path.resolve(file || "", "..", "..");
    info.file = file || "";

    i += h.namesSize - 1;

    // Names is nul-terminated.
    if (data[i] !== 0) throw new Error(`Expected data[${i}] to be 0`);
    i++;
    ...
    Object.keys(info.strings).forEach(function (key) {
      ...
      if (j >= data.length)
        throw new Error(`Expected j to be less than data.length`);
  ...
  parseExtended(data: BufferType) {
    ...
    if (i !== data.length) throw new Error(`Expected i to equal data.length`);
  ...
  _compile(info: any, key: string, str: any) {
    ...
    if (key === "init_file" || key === "reset_file") {
      try {
        str = this.runtime.fs.readFileSync(str, "utf8");
        if (this.debug) {
          v = ("return " + JSON.stringify(str) + ";")
            .replace(/\x1b/g, "\\x1b")
            .replace(/\r/g, "\\r")
            .replace(/\n/g, "\\n");
          this.runtime.process.stdout.write(v + "\n");
        }
        return function () {
          return str;
        };
    ...
    while (val) {
      ...
      if (read(/^%([+\-*\/m&|\^=><])/)) {
        ...
        // For division and modulo, ensure integer results
        var intOp = ch === "/" || ch === "%" ? "|0" : "";
      ...
      if (read(/^%;/)) {
        // Only close if-statement if we're actually inside one
        if (end != null) {
          end = null;
          stmt("}");
        }
        // If not inside if-statement, silently ignore the %; sequence
        continue;
      }
  ...
  _print(code: any, print?: any, done?: any) {
  ...
  print(...args: any[]) {
    ...
    return this._print.apply(
      fake,
      args as [code: any, print?: any, done?: any],
    );
  }
  ...
  readTermcap(term?: string) {
    ...
    if (
      ~term!.indexOf(this.runtime.path.sep) &&
      (terms = this._tryCap(this.runtime.path.resolve(term!)))
    ) {
      term_ = this.runtime.path.basename(term!).split(".")[0];
      if (terms[getEnvVar("TERM")]) {
        term = getEnvVar("TERM");
      } else if (terms[term_]) {
        term = term_;
      } else {
      ...
      for (j = 0; j < fields.length; j++) {
        ...
        if (j === 0) {
          ...
          term = {
            ...
            names: names,
            desc: names.pop(),
            file: ~(file || "").indexOf(this.runtime.path.sep)
              ? this.runtime.path.resolve(file || "")
              : file || "",
            termcap: true,
            bools: {},
            numbers: {},
          ...
          switch (s[i++]) {
            ...
            case "a":
              if (
                ...
                if (s[i + 1] === "p") {
                  getparm(
                    param + s[i + 2].charCodeAt(0) - "@".charCodeAt(0),
                    1,
                  );
                  if (param !== onstack) {
                    pop();
  ...
  getAll() {
    var dir = this._prefix(),
      list = asort(this.runtime.fs.readdirSync(dir)),
      infos: any[] = [];

    list.forEach((letter: string) => {
      var terms = asort(
        this.runtime.fs.readdirSync(this.runtime.path.resolve(dir, letter)),
      );
  ...
  detectUnicode(_info?: any) {
    if (!!getEnvVar("NCURSES_FORCE_UNICODE")) {
      return !!+getEnvVar("NCURSES_FORCE_UNICODE");
    }
  ...
  detectBrokenACS(info: any) {
    ...
    if (!!getEnvVar("NCURSES_NO_UTF8_ACS")) {
      return !!+getEnvVar("NCURSES_NO_UTF8_ACS");
    }
    ...
    if (
      ...
      getEnvVar("TERMCAP") &&
      ~getEnvVar("TERMCAP").indexOf("screen") &&
      ~getEnvVar("TERMCAP").indexOf("hhII00")
    ) {
      if (
  ...
  detectMagicCookie(_info?: any) {
    return !getEnvVar("NCURSES_NO_MAGIC_COOKIE");
  }

  detectPadding(_info?: any) {
    return !getEnvVar("NCURSES_NO_PADDING");
  }

  detectSetbuf(_info?: any) {
    return !getEnvVar("NCURSES_NO_SETBUF");
  }
  ...
  GetConsoleCP() {
    ...
    if (+getEnvVar("NCURSES_NO_WINDOWS_UNICODE") !== 1) {
      return 65001;
    }
    ...
    try {
      ...
      ccp = this.runtime.processes!.childProcess.execFileSync(
        getEnvVar("WINDIR") + "\\system32\\chcp.com",
        [],
        {
  ...
  _prefix(term?: string) {
    ...
    if (term) {
      if (~term.indexOf(this.runtime.path.sep)) {
        return term;
      }
    ...
    // Try exact matches.
    let file = this._tprefix(paths, term);
    if (file) return file;
  ...
  _tprefix(prefix?: any, term?: string, soft?: boolean) {
    ...
    const find = (word: string) => {
      let file = this.runtime.path.resolve(prefix, word[0]);
      try {
        this.runtime.fs.statSync(file);
        return file;
      } catch (e) {
        // ignore
      }
      ...
      file = this.runtime.path.resolve(prefix, ch);
      try {
        this.runtime.fs.statSync(file);
        return file;
      } catch (e) {
    ...
    if (!term) {
      ...
      try {
        dir = this.runtime.fs.readdirSync(prefix).filter((file) => {
    ...
    term = this.runtime.path.basename(term);
    dir = find(term);
    ...
    if (soft) {
      try {
        list = this.runtime.fs.readdirSync(dir);
      } catch (e) {
        return;
      }
      ...
      return sfile && (soft || sdiff === 0)
        ? this.runtime.path.resolve(dir, sfile)
        : null;
    }

    file = this.runtime.path.resolve(dir, term);
    try {
      this.runtime.fs.statSync(file);
      return file;
    } catch (e) {
...
// Lazy getter for ipaths to avoid accessing process.env at module load time
let _ipathsCache: string[] | null = null;
Object.defineProperty(Tput, "ipaths", {
  get() {
    if (!_ipathsCache) {
      _ipathsCache = [
        getEnvVar("TERMINFO"),
        getEnvVar("TERMINFO_DIRS").split(":"),
        getEnvVar("HOME") + "/.terminfo",
        "/usr/share/terminfo",
...
// Lazy getter for cpaths to avoid accessing process.env at module load time
let _cpathsCache: string[] | null = null;
Object.defineProperty(Tput, "cpaths", {
  get() {
    if (!_cpathsCache) {
      _cpathsCache = [
        getEnvVar("TERMCAP"),
        getEnvVar("TERMPATH").split(/[: ]/),
        getEnvVar("HOME") + "/.termcap",
...
function write(data: string) {
  return getRuntime().process.stdout.write(data);
}
...
function tryRead(...files: any[]): string {
  ...
  const resolvedFile = runtime.path.resolve(...files);
  try {
    return runtime.fs.readFileSync(resolvedFile, "utf8");
  } catch (e) {
    return "";
  }
}
...
function sprintf(src: string, ...params: any[]) {
  ...
  return src.replace(
    rule,
    function (_: any, flag: string, widthStr: string, type: string) {
