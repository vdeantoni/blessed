   ...
   _debug(...args: any[]) {
    ...
    parseTerminfo(data: BufferType, file?: string) {
        ...
        Object.keys(info.strings).forEach(function(key) {
            ...
            if (j >= data.length) throw new Error(`Expected j to be less than data.length`);
    ...
    _compile(info: any, key: string, str: any) {
        ...
        while (val) {
            ...
            if (read(/^%([+\-*\/m&|\^=><])/)) {
                ...
                expr('(v = stack.pop(),'
                    + ' stack.push(v = ((stack.pop() ' + ch + ' v)' + intOp + ') || 0),'
                    + ' v)');
                continue;
            }
    ...
    readTermcap(term?: string) {
        ...
        if (~term!.indexOf(this.runtime.path.sep) && (terms = this._tryCap(this.runtime.path.resolve(term!)))) {
    ...
    getAll() {
        ...
        list.forEach((letter: string) => {
            var terms = asort(this.runtime.fs.readdirSync(this.runtime.path.resolve(dir, letter)));
