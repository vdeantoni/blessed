/**
 * bigtext.ts - bigtext element for blessed
 */
...
class BigText extends Box {
  ...
  constructor(options: BigTextOptions = {}) {
    ...
    options.font = options.font || `${dataPath}/fonts/ter-u14n.json`;
    options.fontBold = options.fontBold || `${dataPath}/fonts/ter-u14b.json`;
    ...
    loadFont(filename: string): any {
        ...
        data = JSON.parse(this.runtime.fs.readFileSync(filename, 'utf8') as string);
        ...
        const convertLetter = (_ch: string, lines: any[]): any[] => {
            ...
            lines = lines.map((line: string) => {
                let chs: any[] = line.split('');
                chs = chs.map((ch: string) => {
                    return ch === ' ' ? 0 : 1;
                });
        ...
        font = Object.keys(data.glyphs).reduce((out: any, ch: string) => {
