...
I've improved the runtime initialization experience for @tuxe/node to make it as easy as possible for users. The runtime is now **automatically initialized** when you import from the package - no setup required!
...
import { createScreen } from '@tuxe/node';
const screen = createScreen();

// Pattern 3: Manual runtime (for advanced users)
import { getNodeRuntime, Screen } from '@tuxe/node';
...
**Before:**
```typescript
import { setRuntime, NodeRuntime, Screen } from '@tuxe/node';
...
// In test file
import { Screen } from '@tuxe/node'; // getNodeRuntime() sees existing runtime
...
The runtime initialization is now **invisible** to users - they can focus on building their TUI apps without worrying about setup. The examples demonstrate that @tuxe/node is as simple to use as any other UI library while maintaining the flexibility of the runtime abstraction pattern.
