...
class OverlayImage extends Box {
  ...
  constructor(options: OverlayImageOptions = {}) {
    ...
    renderImage(img: string, ratio: any, callback?: any): any {
        ...
        return self.clearImage(function(err: any) {
            ...
            var ps = self.spawn(OverlayImage.w3mdisplay, [], opt, function(err: any, success: any) {
                if (!callback) return;
                return err
                    ? callback(err)
    ...
    clearImage(callback?: any): any {
        ...
        var ps = this.spawn(OverlayImage.w3mdisplay, [], opt, function(err: any, success: any) {
            if (!callback) return;
            return err
                ? callback(err)
                : callback(null, success);
    ...
    imageSize(callback?: any): any {
        ...
        ps.stdout.on('data', function(data: string) {
            buf += data;
        });
    ...
    getPixelRatio(callback?: any): any {
        ...
        return this.termSize(function(err: any, dimensions: any) {
            if (err) return callback(err);
