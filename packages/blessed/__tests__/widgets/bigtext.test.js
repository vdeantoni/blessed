...
// Store actual fs before mocking using vi.hoisted
const { actualFs } = vi.hoisted(() => {
  const fs = require('fs');
  return { actualFs: { ...fs } };
});

// Mock fs to redirect font paths during tests
vi.mock('fs', () => ({
  default: {
    readFileSync: vi.fn((filePath, encoding) => {
      let correctedPath = filePath;

      // Redirect paths that try to access /lib/widgets/usr/ or /lib/usr/
      if (typeof filePath === 'string') {
        if (filePath.includes('/lib/widgets/usr/')) {
          correctedPath = filePath.replace('/lib/widgets/usr/', '/usr/');
        } else if (filePath.includes('/lib/usr/')) {
          correctedPath = filePath.replace('/lib/usr/', '/usr/');
        }
      }

      return actualFs.readFileSync(correctedPath, encoding);
    }),
...
import BigText from '../../lib/widgets/bigtext.js';
...
describe('BigText', () => {
  ...
  describe('constructor', () => {
    ...
    it('should accept custom font', () => {
      const bigtext = new BigText({
        screen,
        font: __dirname + '/../../usr/fonts/ter-u14n.json'
      });
  ...
  describe('loadFont()', () => {
    ...
    it('should return font object', () => {
      ...
      const font = bigtext.loadFont(__dirname + '/../../usr/fonts/ter-u14n.json');
