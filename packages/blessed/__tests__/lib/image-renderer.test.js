...
import tng, { ImageRenderer } from '../../lib/image-renderer.ts';
...
describe('ImageRenderer', () => {
  ...
  describe('Rendering Methods', () => {
    ...
    it('should render to screen buffer', () => {
      const mockScreen = {
        lines: Array(10).fill(null).map(() =>
          Array(10).fill(null).map(() => [0, ' '])
        )
      };
    ...
    it('should handle alpha blending in renderScreen', () => {
      ...
      const mockScreen = {
        lines: Array(5).fill(null).map(() =>
          Array(5).fill(null).map(() => [0x1ff, ' '])
        )
      };

      transparentImg.renderScreen(transparentImg.cellmap, mockScreen, 0, 5, 0, 5);
