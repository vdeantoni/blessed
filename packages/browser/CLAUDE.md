# Claude Context for @tuxe/browser

This document provides architectural context and development guidelines for the `@tuxe/browser` package.

## Overview

`@tuxe/browser` is a **browser runtime adapter** that enables @tuxe/core TUI applications to run in web browsers. It provides:
...
- Full widget support - all @tuxe/core widgets work unchanged
...
`@tuxe/browser` bridges `@tuxe/core` (platform-agnostic) to the browser environment by:
...
- Re-exports all @tuxe/core widgets
...
The runtime MUST be initialized **before** any @tuxe/core modules load:
...
console.log('[tuxe-browser] Runtime initialized');
```

**Why?** When `export * from '@tuxe/core'` executes in index.ts, widget modules load and some access `getRuntime()` during initialization. Without prior setup, you get "Runtime not initialized" errors.
...
Implements a minimal Program-like interface for @tuxe/core:
...
```typescript
import xtermData from '@tuxe/core/data/terminfo/xterm-256color.json';
import terU14n from '@tuxe/core/data/fonts/ter-u14n.json';
import terU14b from '@tuxe/core/data/fonts/ter-u14b.json';
...
**Solution:** Runtime auto-initializes when you import from `@tuxe/browser`. Ensure you import from `@tuxe/browser` (not `@tuxe/core` directly) at the entry point of your application.
...
If @tuxe/core needs a new Node.js API:

1. **Add to Runtime interface** in @tuxe/core:
```typescript
...
- `tuxe-page.ts` - Page object for common operations
...
- Keep runtime adapter thin - complex logic in @tuxe/core
...
```typescript
import { getRuntime } from '@tuxe/core';
...
1. Check if @tuxe/browser is imported (runtime auto-initializes on import)
...
The key to @tuxe/browser is understanding the **initialization order**:

1. Global polyfills set up (process, Buffer)
2. Runtime created and registered
3. @tuxe/core modules loaded
4. XTerm adapter bridges to browser
5. Widgets work unchanged!

Always remember: **Runtime MUST be ready BEFORE any @tuxe/core module tries to use it.**
