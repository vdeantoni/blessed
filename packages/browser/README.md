...
`@tuxe/browser` enables terminal user interface (TUI) applications built with `@tuxe/core` to run in web browsers. It provides:

- **Browser polyfills** for Node.js APIs (fs, process, path, etc.)
- **xterm.js integration** for terminal rendering
- **Runtime adapter** that bridges @tuxe/core to the browser environment
- **Full widget support** - all @tuxe/core widgets work in the browser

## Installation

```bash
npm install @tuxe/browser xterm
# or
pnpm add @tuxe/browser xterm
# or
yarn add @tuxe/browser xterm
```
...
<head>
  ...
  <title>Tuxe Browser Example</title>
...
// Runtime is automatically initialized when you import @tuxe/browser
```

> **Note:** The runtime is automatically initialized when you import from `@tuxe/browser`. You don't need to call any initialization functions.

### Widget Re-exports

All @tuxe/core widgets are re-exported for convenience:
...
All @tuxe/core widgets work in the browser:
...
```typescript
import { Form, Input, Button } from '@tuxe/browser';
...
```typescript
import { FileManager } from '@tuxe/browser';
...
1. **Runtime Abstraction**: @tuxe/core uses a runtime interface for all platform operations
2. **Browser Runtime**: This package provides browser-compatible implementations
3. **Automatic Initialization**: Runtime is set up when you import @tuxe/browser
